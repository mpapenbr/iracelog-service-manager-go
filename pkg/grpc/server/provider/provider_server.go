...
func WithPubSubData(arg pubsub.PubSubData) Option {
	return func(srv *providerServer) {
		srv.pubsubData = arg
	}
}
...
func (s *providerServer) ListLiveEvents(
	...
	for _, v := range s.pubsubData.LiveEvents() {
...
func (s *providerServer) RegisterEvent(
	...
	if ed, _ := s.pubsubData.GetEvent(selector); ed != nil {
	...
	s.pubsubData.PublishEventRegistered(&pubsub.EventData{
		Event: req.Msg.Event,
		Track: dbTrack,
	})
...
func (s *providerServer) UnregisterEvent(
	...
	var ed *pubsub.EventData
	var err error
	if ed, err = s.pubsubData.GetEvent(req.Msg.EventSelector); err != nil {
	...
	s.pubsubData.PublishEventUnregistered(ed.Event.Key)
