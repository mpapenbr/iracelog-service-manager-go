...
func (pd *predictData) PredictParam() (*predictv1.PredictParam, error) {
	toDur := func(sec float32) *durationpb.Duration {
		return durationpb.New(time.Duration(sec * float32(time.Second)))
	}
	if len(pd.myData.stints) < 2 {
		pd.l.Info("not enough data to create PredictParam")
	...
	stintParam := predictv1.StintParam{
		...
		Lps:        int32(pd.myData.stints[len(pd.myData.stints)-2].numLaps),
	}
	calcPitLaneTime := func() float32 {
...
//nolint:lll,funlen,gocritic // readability,wip
func (pd *predictData) ExpertCalcParams() *racestints.ExpertCalcParams {
	...
	lastStint := pd.myData.stints[len(pd.myData.stints)-2]
	ret := &racestints.ExpertCalcParams{
		...
		PitTime: time.Duration(pd.myData.pits[len(pd.myData.pits)-1].timeUsed * float32(time.Second)),
		AvgLap:  time.Duration(lastStint.rAvg * float32(time.Second)),
		EndOfLapDataFunc: func() *racestints.EndOfLapData {
			...
			return &racestints.EndOfLapData{
				...
				SessionAtEol: time.Duration((remainLapTime) * float32(time.Second)),
			}
		},
	}
	return ret
