...
func NewCheckSnapshotsBobCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "snapshot-bob eventId",
...
func checkSnapshotBob(eventArg string) {
	log.Debug("checkSnapshotBob called")
	...
	data, err := smRepo.LoadSnapshots(ctx, eventID, 300)
	if err != nil {
		log.Error("failed to load snapshots", log.ErrorField(err))
		return
	}
