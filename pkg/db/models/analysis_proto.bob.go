...
var AnalysisProtos = psql.NewTablex[*AnalysisProto, AnalysisProtoSlice, *AnalysisProtoSetter]("", "analysis_proto", buildAnalysisProtoColumns("analysis_proto"), analysisProtoScanMapper)
...
func analysisProtoScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*AnalysisProto, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *AnalysisProto) any
	}
	targets := make([]target, 0, 3)
	for i, col := range cols {
		switch col {
		case "event_id":
			targets = append(targets, target{i, func(o *AnalysisProto) any { return &o.EventID }})
		case "record_stamp":
			targets = append(targets, target{i, func(o *AnalysisProto) any { return &o.RecordStamp }})
		case "protodata":
			targets = append(targets, target{i, func(o *AnalysisProto) any { return &o.Protodata }})
		}
	}
	...
	return func(row *scan.Row) (any, error) {
			...
			return o, nil
		}, func(v any) (*AnalysisProto, error) {
			return v.(*AnalysisProto), nil
		}
}
...
func (o AnalysisProtoSlice) copyMatchingRows(from ...*AnalysisProto) {
	fromByPK := make(map[int32]*AnalysisProto, len(from))
...
func buildAnalysisProtoWhere[Q psql.Filterable](cols analysisProtoColumns) analysisProtoWhere[Q] {
	return analysisProtoWhere[Q]{
		...
		R:           analysisProtoWhereR[Q]{cols: cols},
	}
}

// analysisProtoWhereR holds the relationship-based filters of
// analysisProtoWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type analysisProtoWhereR[Q psql.Filterable] struct {
	cols analysisProtoColumns
}
...
func (w analysisProtoWhereR[Q]) HasEvent(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
...
// NULL-tolerant scan buffer for analysisProtoScanMapperNullable:
...
type analysisProtoPreloadBuf struct {
	EventID     null.Val[int32]
	RecordStamp null.Val[time.Time]
	Protodata   null.Val[[]byte]
}

// analysisProtoScanMapperNullable maps the preloaded analysisProto
...
func analysisProtoScanMapperNullable(prefix string) scan.Mapper[*AnalysisProto] {
	return func(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*AnalysisProto, error)) {
		// resolve the column names once per query, not per row
		type target struct {
			idx int
			dst func(b *analysisProtoPreloadBuf) any
		}
		targets := make([]target, 0, 3)
		for i, col := range cols {
			...
			switch name {
			case "event_id":
				targets = append(targets, target{i, func(b *analysisProtoPreloadBuf) any { return &b.EventID }})
			case "record_stamp":
				targets = append(targets, target{i, func(b *analysisProtoPreloadBuf) any { return &b.RecordStamp }})
			case "protodata":
				targets = append(targets, target{i, func(b *analysisProtoPreloadBuf) any { return &b.Protodata }})
			}
		}
		...
		buf := new(analysisProtoPreloadBuf)
		return func(row *scan.Row) (any, error) {
				for _, t := range targets {
					row.ScheduleScanByIndex(t.idx, t.dst(buf))
				}
				return buf, nil
			}, func(link any) (*AnalysisProto, error) {
				buf := link.(*analysisProtoPreloadBuf)
				...
				if buf.Protodata.IsValue() {
					o.Protodata = buf.Protodata.MustGet()
				}
				return o, nil
			}
	}
