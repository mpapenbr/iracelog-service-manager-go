...
var CCarTeams = psql.NewTablex[*CCarTeam, CCarTeamSlice, *CCarTeamSetter]("", "c_car_team", buildCCarTeamColumns("c_car_team"), cCarTeamScanMapper)
...
type cCarTeamR struct {
	...
	Loaded cCarTeamRLoaded `db:"-" `
}

// cCarTeamRLoaded tracks which relationships on CCarTeam have been loaded.
type cCarTeamRLoaded struct {
	CCarEntry bool // c_car_team.c_car_team_car_entry_id_fkey
}

func buildCCarTeamColumns(tableName string) cCarTeamColumns {
	...
	return cCarTeamColumns{
		...
		ID:          buildCCarTeamColumn(tableName, "id"),
		CCarEntryID: buildCCarTeamColumn(tableName, "c_car_entry_id"),
		TeamID:      buildCCarTeamColumn(tableName, "team_id"),
		Name:        buildCCarTeamColumn(tableName, "name"),
	}
}
...
type cCarTeamColumns struct {
	...
	tableAlias  string
	ID          cCarTeamColumn
	CCarEntryID cCarTeamColumn
	TeamID      cCarTeamColumn
	Name        cCarTeamColumn
}
...
func (cCarTeamColumns) AliasedAs(tableName string) cCarTeamColumns {
	return buildCCarTeamColumns(tableName)
}

// Unqualified returns a copy of the columns set without table qualification.
func (c cCarTeamColumns) Unqualified() cCarTeamColumns {
	return buildCCarTeamColumns("")
}

func buildCCarTeamColumn(alias, name string) cCarTeamColumn {
	return cCarTeamColumn{
...
func (c cCarTeamColumn) Name() string {
	return c.name
}

// ShouldOmitParens prevents automatic parenthesis wrapping in expression builders.
func (c cCarTeamColumn) ShouldOmitParens() bool {
	return true
}
...
func cCarTeamScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*CCarTeam, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *CCarTeam) any
	}
	targets := make([]target, 0, 4)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *CCarTeam) any { return &o.ID }})
		case "c_car_entry_id":
			targets = append(targets, target{i, func(o *CCarTeam) any { return &o.CCarEntryID }})
		case "team_id":
			targets = append(targets, target{i, func(o *CCarTeam) any { return &o.TeamID }})
		case "name":
			targets = append(targets, target{i, func(o *CCarTeam) any { return &o.Name }})
		}
	}
	...
	return func(row *scan.Row) (any, error) {
			...
			return o, nil
		}, func(v any) (*CCarTeam, error) {
			return v.(*CCarTeam), nil
		}
}
...
func (o *CCarTeam) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	...
	case bob.QueryTypeMerge:
		ctx, err = CCarTeams.AfterMergeHooks.RunHooks(ctx, exec, CCarTeamSlice{o})
	}
...
func (o *CCarTeam) pkEQ() dialect.Expression {
	return psql.Quote("c_car_team", "id").EQ(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
...
func (o CCarTeamSlice) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	...
	case bob.QueryTypeMerge:
		ctx, err = CCarTeams.AfterMergeHooks.RunHooks(ctx, exec, o)
	}
...
func (o CCarTeamSlice) pkIN() dialect.Expression {
	...
	return psql.Quote("c_car_team", "id").In(bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
...
func (o CCarTeamSlice) copyMatchingRows(from ...*CCarTeam) {
	fromByPK := make(map[int32]*CCarTeam, len(from))
...
func (o CCarTeamSlice) MergeMod() bob.Mod[*dialect.MergeQuery] {
	return bob.ModFunc[*dialect.MergeQuery](func(q *dialect.MergeQuery) {
		q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
			return CCarTeams.BeforeMergeHooks.RunHooks(ctx, exec, o)
		})
		...
		q.AppendLoader(bob.LoaderFunc(func(ctx context.Context, exec bob.Executor, retrieved any) error {
			...
			default:
				...
				_, err = CCarTeams.AfterMergeHooks.RunHooks(ctx, exec, o)
			}
...
func (cCarTeam0 *CCarTeam) InsertCCarEntry(ctx context.Context, exec bob.Executor, related *CCarEntrySetter) error {
	...
	cCarTeam0.R.Loaded.CCarEntry = true
...
func buildCCarTeamWhere[Q psql.Filterable](cols cCarTeamColumns) cCarTeamWhere[Q] {
	return cCarTeamWhere[Q]{
		...
		R:           cCarTeamWhereR[Q]{cols: cols},
	}
}

// cCarTeamWhereR holds the relationship-based filters of
// cCarTeamWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type cCarTeamWhereR[Q psql.Filterable] struct {
	cols cCarTeamColumns
}
...
func (w cCarTeamWhereR[Q]) HasCCarEntry(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
...
func (os CCarTeamSlice) LoadCCarEntry(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	...
	cCarTeamByKey := make(map[int32][]*CCarTeam, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		cCarTeamByKey[o.CCarEntryID] = append(cCarTeamByKey[o.CCarEntryID], o)
	}

	for _, rel := range cCarEntries {

		owners, ok := cCarTeamByKey[rel.ID]
		if !ok {
			continue
		}
