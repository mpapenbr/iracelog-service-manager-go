...
var SchemaMigrations = psql.NewTablex[*SchemaMigration, SchemaMigrationSlice, *SchemaMigrationSetter]("", "schema_migrations", buildSchemaMigrationColumns("schema_migrations"), schemaMigrationScanMapper)
...
func buildSchemaMigrationColumns(tableName string) schemaMigrationColumns {
	...
	return schemaMigrationColumns{
		...
		Version:     buildSchemaMigrationColumn(tableName, "version"),
		Dirty:       buildSchemaMigrationColumn(tableName, "dirty"),
	}
}
...
type schemaMigrationColumns struct {
	expr.ColumnsExpr
	tableAlias string
	Version    schemaMigrationColumn
	Dirty      schemaMigrationColumn
}
...
func buildSchemaMigrationColumn(alias, name string) schemaMigrationColumn {
	return schemaMigrationColumn{
...
func (c schemaMigrationColumn) Name() string {
	return c.name
}

// ShouldOmitParens prevents automatic parenthesis wrapping in expression builders.
func (c schemaMigrationColumn) ShouldOmitParens() bool {
	return true
}
...
func (s *SchemaMigrationSetter) Apply(q *dialect.InsertQuery) {
	...
	q.AppendValues(
		bob.ExpressionFunc(func(ctx context.Context, w io.StringWriter, d bob.Dialect, start int) ([]any, error) {
			...
			return psql.Arg(s.Dirty.MustGet()).WriteSQL(ctx, w, d, start)
		}))
}
...
func schemaMigrationScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*SchemaMigration, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *SchemaMigration) any
	}
	targets := make([]target, 0, 2)
	for i, col := range cols {
		switch col {
		case "version":
			targets = append(targets, target{i, func(o *SchemaMigration) any { return &o.Version }})
		case "dirty":
			targets = append(targets, target{i, func(o *SchemaMigration) any { return &o.Dirty }})
		}
	}

	return func(row *scan.Row) (any, error) {
			o := new(SchemaMigration)
			for _, t := range targets {
				row.ScheduleScanByIndex(t.idx, t.dst(o))
			}
			return o, nil
		}, func(v any) (*SchemaMigration, error) {
			return v.(*SchemaMigration), nil
		}
}
...
func (o *SchemaMigration) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	...
	case bob.QueryTypeMerge:
		ctx, err = SchemaMigrations.AfterMergeHooks.RunHooks(ctx, exec, SchemaMigrationSlice{o})
	}
...
func (o SchemaMigrationSlice) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	...
	case bob.QueryTypeMerge:
		ctx, err = SchemaMigrations.AfterMergeHooks.RunHooks(ctx, exec, o)
	}
...
func (o SchemaMigrationSlice) copyMatchingRows(from ...*SchemaMigration) {
	fromByPK := make(map[int64]*SchemaMigration, len(from))
	for _, new := range from {
		// keep the first row for each key, like the nested loop did
		if _, ok := fromByPK[new.Version]; !ok {
			fromByPK[new.Version] = new
		}
	}

	for i, old := range o {
		new, ok := fromByPK[old.Version]
		if !ok {
			continue
		}
...
func (o SchemaMigrationSlice) MergeMod() bob.Mod[*dialect.MergeQuery] {
	return bob.ModFunc[*dialect.MergeQuery](func(q *dialect.MergeQuery) {
		q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
			return SchemaMigrations.BeforeMergeHooks.RunHooks(ctx, exec, o)
		})
		...
		q.AppendLoader(bob.LoaderFunc(func(ctx context.Context, exec bob.Executor, retrieved any) error {
			...
			default:
				...
				_, err = SchemaMigrations.AfterMergeHooks.RunHooks(ctx, exec, o)
			}
...
func buildSchemaMigrationWhere[Q psql.Filterable](cols schemaMigrationColumns) schemaMigrationWhere[Q] {
	return schemaMigrationWhere[Q]{
		cols:    cols,
		Version: psql.Where[Q, int64](cols.Version.Expression),
		Dirty:   psql.Where[Q, bool](cols.Dirty.Expression),
	}
}
