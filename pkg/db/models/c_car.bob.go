...
type cCarRLoaded struct {
	CCarClass   bool // c_car.c_car_class_id_fkey
...
func (os CCarSlice) CCarClass(mods ...bob.Mod[*dialect.SelectQuery]) CCarClassesQuery {
	...
	PKArgExpr := psql.Any(psql.Cast(psql.Arg(pkCCarClassID), "integer[]"))

	return CCarClasses.Query(append(mods,
		sm.Where(CCarClasses.Columns.ID.EQ(PKArgExpr)),
	)...)
}
...
// HasCCarClass filters parents that have a matching CCarClass using a
...
func (w cCarWhereR[Q]) HasCCarClass(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
		sm.Columns(psql.Raw("1")),
		sm.From(CCarClasses.NameExpr()),
		sm.Where(CCarClasses.Columns.ID.EQ(w.cols.CCarClassID)),
	)
	q.Apply(filters...)
...
func buildCCarPreloader() cCarPreloader {
	return cCarPreloader{
		CCarClass: func(opts ...psql.PreloadOption) psql.Preloader {
			...
			}, CCarClasses.Columns.Names(), cCarClassScanMapperNullable, opts...)
		},
...
func (os CCarSlice) LoadCCarClass(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	...
	for _, o := range os {
		...
		cCarByKey[o.CCarClassID] = append(cCarByKey[o.CCarClassID], o)
	}
