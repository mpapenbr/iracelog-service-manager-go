...
var Tracks = psql.NewTablex[*Track, TrackSlice, *TrackSetter]("", "track")
...
var TrackColumns = buildTrackColumns("track")
...
var TrackErrors = &trackErrors{
	ErrUniqueTrackPkey: &UniqueConstraintError{
		schema:  "",
		table:   "track",
		columns: []string{"id"},
		s:       "track_pkey",
...
func (o *Track) Events(mods ...bob.Mod[*dialect.SelectQuery]) EventsQuery {
	return Events.Query(append(mods,
		sm.Where(EventColumns.TrackID.EQ(psql.Arg(o.ID))),
	)...)
}
...
func (os TrackSlice) Events(mods ...bob.Mod[*dialect.SelectQuery]) EventsQuery {
	...
	return Events.Query(append(mods,
		sm.Where(psql.Group(EventColumns.TrackID).OP("IN", PKArgExpr)),
	)...)
}
