...
var MSGStateProtos = psql.NewTablex[*MSGStateProto, MSGStateProtoSlice, *MSGStateProtoSetter]("", "msg_state_proto", buildMSGStateProtoColumns("msg_state_proto"), msgStateProtoScanMapper)
...
type msgStateProtoR struct {
	...
	Loaded msgStateProtoRLoaded `db:"-" `
}

// msgStateProtoRLoaded tracks which relationships on MSGStateProto have been loaded.
type msgStateProtoRLoaded struct {
	RSInfo bool // msg_state_proto.msg_state_proto_rs_info_id_fkey
}

func buildMSGStateProtoColumns(tableName string) msgStateProtoColumns {
	...
	return msgStateProtoColumns{
		...
		ID:          buildMSGStateProtoColumn(tableName, "id"),
		RSInfoID:    buildMSGStateProtoColumn(tableName, "rs_info_id"),
		Protodata:   buildMSGStateProtoColumn(tableName, "protodata"),
	}
}
...
type msgStateProtoColumns struct {
	expr.ColumnsExpr
	tableAlias string
	ID         msgStateProtoColumn
	RSInfoID   msgStateProtoColumn
	Protodata  msgStateProtoColumn
}
...
func (msgStateProtoColumns) AliasedAs(tableName string) msgStateProtoColumns {
	return buildMSGStateProtoColumns(tableName)
}

// Unqualified returns a copy of the columns set without table qualification.
func (c msgStateProtoColumns) Unqualified() msgStateProtoColumns {
	return buildMSGStateProtoColumns("")
}

func buildMSGStateProtoColumn(alias, name string) msgStateProtoColumn {
	return msgStateProtoColumn{
...
type msgStateProtoColumn struct {
	psql.Expression
	alias string
	name  string
}
...
func (c msgStateProtoColumn) Name() string {
	return c.name
}

// ShouldOmitParens prevents automatic parenthesis wrapping in expression builders.
func (c msgStateProtoColumn) ShouldOmitParens() bool {
	return true
}
...
func msgStateProtoScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*MSGStateProto, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *MSGStateProto) any
	}
	targets := make([]target, 0, 3)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *MSGStateProto) any { return &o.ID }})
		case "rs_info_id":
			targets = append(targets, target{i, func(o *MSGStateProto) any { return &o.RSInfoID }})
		case "protodata":
			targets = append(targets, target{i, func(o *MSGStateProto) any { return &o.Protodata }})
		}
	}
	...
	return func(row *scan.Row) (any, error) {
			...
			return o, nil
		}, func(v any) (*MSGStateProto, error) {
			return v.(*MSGStateProto), nil
		}
}
...
func (o *MSGStateProto) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	...
	case bob.QueryTypeMerge:
		ctx, err = MSGStateProtos.AfterMergeHooks.RunHooks(ctx, exec, MSGStateProtoSlice{o})
	}
...
func (o MSGStateProtoSlice) AfterQueryHook(ctx context.Context, exec bob.Executor, queryType bob.QueryType) error {
	...
	case bob.QueryTypeMerge:
		ctx, err = MSGStateProtos.AfterMergeHooks.RunHooks(ctx, exec, o)
	}
...
func (o MSGStateProtoSlice) copyMatchingRows(from ...*MSGStateProto) {
	fromByPK := make(map[int32]*MSGStateProto, len(from))
...
func (o MSGStateProtoSlice) MergeMod() bob.Mod[*dialect.MergeQuery] {
	return bob.ModFunc[*dialect.MergeQuery](func(q *dialect.MergeQuery) {
		q.AppendHooks(func(ctx context.Context, exec bob.Executor) (context.Context, error) {
			return MSGStateProtos.BeforeMergeHooks.RunHooks(ctx, exec, o)
		})
		...
		q.AppendLoader(bob.LoaderFunc(func(ctx context.Context, exec bob.Executor, retrieved any) error {
			...
			default:
				...
				_, err = MSGStateProtos.AfterMergeHooks.RunHooks(ctx, exec, o)
			}
...
func (msgStateProto0 *MSGStateProto) InsertRSInfo(ctx context.Context, exec bob.Executor, related *RSInfoSetter) error {
	...
	msgStateProto0.R.Loaded.RSInfo = true
...
func buildMSGStateProtoWhere[Q psql.Filterable](cols msgStateProtoColumns) msgStateProtoWhere[Q] {
	return msgStateProtoWhere[Q]{
		...
		R:         msgStateProtoWhereR[Q]{cols: cols},
	}
}

// msgStateProtoWhereR holds the relationship-based filters of
// msgStateProtoWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type msgStateProtoWhereR[Q psql.Filterable] struct {
	cols msgStateProtoColumns
}
...
func (w msgStateProtoWhereR[Q]) HasRSInfo(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
...
func (os MSGStateProtoSlice) LoadRSInfo(ctx context.Context, exec bob.Executor, mods ...bob.Mod[*dialect.SelectQuery]) error {
	...
	msgStateProtoByKey := make(map[int32][]*MSGStateProto, len(os))
	for _, o := range os {
		if o == nil {
			continue
		}

		msgStateProtoByKey[o.RSInfoID] = append(msgStateProtoByKey[o.RSInfoID], o)
	}

	for _, rel := range rsInfos {

		owners, ok := msgStateProtoByKey[rel.ID]
		if !ok {
			continue
		}
