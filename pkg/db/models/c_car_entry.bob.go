...
var CCarEntries = psql.NewTablex[*CCarEntry, CCarEntrySlice, *CCarEntrySetter]("", "c_car_entry", buildCCarEntryColumns("c_car_entry"), cCarEntryScanMapper)
...
func cCarEntryScanMapper(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*CCarEntry, error)) {
	// resolve the column positions once per query, not per row
	type target struct {
		idx int
		dst func(o *CCarEntry) any
	}
	targets := make([]target, 0, 6)
	for i, col := range cols {
		switch col {
		case "id":
			targets = append(targets, target{i, func(o *CCarEntry) any { return &o.ID }})
		case "event_id":
			targets = append(targets, target{i, func(o *CCarEntry) any { return &o.EventID }})
		case "c_car_id":
			targets = append(targets, target{i, func(o *CCarEntry) any { return &o.CCarID }})
		case "car_idx":
			targets = append(targets, target{i, func(o *CCarEntry) any { return &o.CarIdx }})
		case "car_number":
			targets = append(targets, target{i, func(o *CCarEntry) any { return &o.CarNumber }})
		case "car_number_raw":
			targets = append(targets, target{i, func(o *CCarEntry) any { return &o.CarNumberRaw }})
		}
	}
	...
	return func(row *scan.Row) (any, error) {
			...
			return o, nil
		}, func(v any) (*CCarEntry, error) {
			return v.(*CCarEntry), nil
		}
}
...
func (o CCarEntrySlice) copyMatchingRows(from ...*CCarEntry) {
	fromByPK := make(map[int32]*CCarEntry, len(from))
...
func buildCCarEntryWhere[Q psql.Filterable](cols cCarEntryColumns) cCarEntryWhere[Q] {
	return cCarEntryWhere[Q]{
		...
		R:            cCarEntryWhereR[Q]{cols: cols},
	}
}

// cCarEntryWhereR holds the relationship-based filters of
// cCarEntryWhere under the R namespace — mirroring the model's
// R struct — so they cannot collide with column-based filter fields.
type cCarEntryWhereR[Q psql.Filterable] struct {
	cols cCarEntryColumns
}

// HasCCarDrivers filters parents that have a matching CCarDrivers using a
...
// applied to the subquery (i.e. to CCarDrivers).
func (w cCarEntryWhereR[Q]) HasCCarDrivers(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
		sm.Columns(psql.Raw("1")),
		sm.From(CCarDrivers.NameExpr()),
		sm.Where(CCarDrivers.Columns.CCarEntryID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: psql.Exists(q)}
}

// HasCCar filters parents that have a matching CCar using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to CCars).
func (w cCarEntryWhereR[Q]) HasCCar(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
		sm.Columns(psql.Raw("1")),
		sm.From(CCars.NameExpr()),
		sm.Where(CCars.Columns.ID.EQ(w.cols.CCarID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: psql.Exists(q)}
}
...
func (w cCarEntryWhereR[Q]) HasEvent(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
		sm.Columns(psql.Raw("1")),
...
// HasCCarTeams filters parents that have a matching CCarTeams using a
// correlated EXISTS subquery (semi-join). Unlike an INNER JOIN it does not
// multiply parent rows, so no DISTINCT is needed. The optional filters are
// applied to the subquery (i.e. to CCarTeams).
func (w cCarEntryWhereR[Q]) HasCCarTeams(filters ...bob.Mod[*dialect.SelectQuery]) mods.Where[Q] {
	q := psql.Select(
		sm.Columns(psql.Raw("1")),
		sm.From(CCarTeams.NameExpr()),
		sm.Where(CCarTeams.Columns.CCarEntryID.EQ(w.cols.ID)),
	)
	q.Apply(filters...)
	return mods.Where[Q]{E: psql.Exists(q)}
}

// NULL-tolerant scan buffer for cCarEntryScanMapperNullable:
...
type cCarEntryPreloadBuf struct {
	ID           null.Val[int32]
	EventID      null.Val[int32]
	CCarID       null.Val[int32]
	CarIdx       null.Val[int32]
	CarNumber    null.Val[string]
	CarNumberRaw null.Val[int32]
}

// cCarEntryScanMapperNullable maps the preloaded cCarEntry
...
func cCarEntryScanMapperNullable(prefix string) scan.Mapper[*CCarEntry] {
	return func(ctx context.Context, cols []string) (scan.BeforeFunc, func(any) (*CCarEntry, error)) {
		// resolve the column names once per query, not per row
		type target struct {
			idx int
			dst func(b *cCarEntryPreloadBuf) any
		}
		targets := make([]target, 0, 6)
		for i, col := range cols {
			...
			switch name {
			case "id":
				targets = append(targets, target{i, func(b *cCarEntryPreloadBuf) any { return &b.ID }})
			case "event_id":
				targets = append(targets, target{i, func(b *cCarEntryPreloadBuf) any { return &b.EventID }})
			case "c_car_id":
				targets = append(targets, target{i, func(b *cCarEntryPreloadBuf) any { return &b.CCarID }})
			case "car_idx":
				targets = append(targets, target{i, func(b *cCarEntryPreloadBuf) any { return &b.CarIdx }})
			case "car_number":
				targets = append(targets, target{i, func(b *cCarEntryPreloadBuf) any { return &b.CarNumber }})
			case "car_number_raw":
				targets = append(targets, target{i, func(b *cCarEntryPreloadBuf) any { return &b.CarNumberRaw }})
			}
		}
		...
		buf := new(cCarEntryPreloadBuf)
		return func(row *scan.Row) (any, error) {
				for _, t := range targets {
					row.ScheduleScanByIndex(t.idx, t.dst(buf))
				}
				return buf, nil
			}, func(link any) (*CCarEntry, error) {
				buf := link.(*cCarEntryPreloadBuf)
				...
				if !(buf.ID.IsValue()) &&
					...
					!(buf.CCarID.IsValue()) &&
					!(buf.CarIdx.IsValue()) &&
					!(buf.CarNumber.IsValue()) &&
					!(buf.CarNumberRaw.IsValue()) {
					return nil, nil
				}
				...
				if buf.CCarID.IsValue() {
					o.CCarID = buf.CCarID.MustGet()
				}
				if buf.CarIdx.IsValue() {
					o.CarIdx = buf.CarIdx.MustGet()
				}
				if buf.CarNumber.IsValue() {
					o.CarNumber = buf.CarNumber.MustGet()
				}
				if buf.CarNumberRaw.IsValue() {
					o.CarNumberRaw = buf.CarNumberRaw.MustGet()
				}
				return o, nil
			}
	}
...
func buildCCarEntryPreloader() cCarEntryPreloader {
	return cCarEntryPreloader{
		CCar: func(opts ...psql.PreloadOption) psql.Preloader {
			...
			}, CCars.Columns.Names(), cCarScanMapperNullable, opts...)
		},
