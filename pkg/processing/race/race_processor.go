...
func (p *RaceProcessor) ProcessStatePayload(stateMsg *model.StateData) {
	...
	for _, carClass := range p.carProcessor.CarClasses {
		p.processClassRaceGraph(&stateMsg.Payload, carClass)
	}
...
func (p *RaceProcessor) processClassRaceGraph(
	...
	// this will be the new entry for that lap
	raceGraphLapEntry := model.AnalysisRaceGraph{
		LapNo:    p.getInt(carClassEntries[leaderEntry], "lc"),
...
func (p *RaceProcessor) filterByCarClass(
	...
	for i := range payload.Cars {
		carIdx := p.getInt(payload.Cars[i], "carIdx")
		if p.carProcessor.ByCarIdx[carIdx].Car.CarClassID == carClass.ID {
