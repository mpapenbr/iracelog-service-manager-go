...
PROJECTNAME="iracelog-service-manager-go"
...
.PHONY: install
## `install`: Fetch dependencies needed to run `iracelog-service-manager-go`
install:
...
.PHONY: --test-runner
--test-runner:
	go test  `go list ./... | grep pkg/grpc` -p 1 -race -covermode=atomic -coverprofile=./coverage/coverage.txt
...
.PHONY: run
## :
## `run`: Run `iracelog-service-manager-go` in production mode
...
.PHONY: run-debug
## `run-debug`: Run `iracelog-service-manager-go` in debug mode
